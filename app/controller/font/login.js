...
class Login extends Controller {
    ...
    async loginValidated() {
        ...
        if (accountType == ACCOUNT_TYPE.mobileCode) {
            ...
            try {
                const mobileCode = ctx.session.lastMobileCode[account].mobileCode;
    ...
    async getMobileCode() {
        ...
        const mobileCode = Math.random().toString().slice(-6);
        const currentTime = (new Date()).getTime();// 获取当前时间戳
        const lastMobileCode = ctx.session.lastMobileCode;// 上一次获取到的验证码
        try {
